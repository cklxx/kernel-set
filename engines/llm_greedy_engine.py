#!/usr/bin/env python3
"""Single-request text LLM greedy decode engine built from kernel-set primitives.
...
measurement and reporting. It is model-family neutral for Hugging Face
Llama/Qwen-style decoder-only LLMs, but it is not the VLM or diffusion engine.
"""
...
def _resolve_core(model):
    ...
    if hasattr(model, "layers") and hasattr(model, "embed_tokens"):
        return model
    raise TypeError(
        "KernelSetLLMEngine expects a HF decoder-only LLM core with layers and embed_tokens"
    )
...
class KernelSetLLMFullPath:
    """Single-request Llama/Qwen-style decoder-only LLM path using kernel-set kernels."""
...
class KernelSetLLMConfigurablePath(KernelSetLLMFullPath):
    """Single-request text LLM path with per-component ks/torch switches."""

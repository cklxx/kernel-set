...
- `gpu_caps.json` — the single source of truth for GPU SM/capabilities and dtype
  aliases, shared with the runtime dispatch probe in `bindings/.../backends/`.
...
> **Three-tier routing.** `select.py`/`ksctl` is the *static model→kernel plan*
> (Tier 1); `kernel_set.dispatch` + `backends/` is the *runtime
> best-available-provider selection* (Tier 2); the kernel-set C ABI is the
> *portable fallback* (Tier 3). See [`ROUTING.md`](ROUTING.md) for the full map
> of which module owns what and how the catalogs relate.

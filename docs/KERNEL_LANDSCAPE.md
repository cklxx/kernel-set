# Kernel Landscape — High-Performance LLM Kernels (2026 H1)

The single canonical survey of the real-world landscape of high-performance
kernels for LLM inference and training. It is the provenance + sourcing
reference for `kernel-set`: every operator we expose is modeled on the
algorithms documented here (clean-room implementations), and it is the source
for [`benchmarks/baselines.yaml`](../benchmarks/baselines.yaml) — the
machine-readable config the bench uses to install real upstream kernels and
compare them, op-for-op, against kernel-set.

This doc has two layers:

- **What's new and fast right now** — the freshly-researched **2026 H1** library
  catalog (sections 1–3 below), high/medium confidence only, as of
  **2026-06-04**.
- **The standing survey** — the slower-moving reference: **best-in-class kernel
  per operator** (by arch/dtype) and the **model-family classification**
  (sections 4–5 below).

For the full ranked per-op catalog with every provider, see
...
The slower-moving reference layer: the best-in-class kernel per operator (by
arch/dtype) and the model-family classification. Use this to reason about *which*
kernel a given (model, GPU, dtype) should route to; use sections 1–3 above for
*what's new* in 2026 H1.

## 4. Best-in-class kernel per operator
